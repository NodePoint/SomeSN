...
if(isset($_GET['id'])) {
	...
	$idget = htmlentities($idget, ENT_QUOTES); // DON'T NEED THIS
}
else {
	$idget = '';
}
$idgetch = mysqli_query($conn, "SELECT id FROM status_log WHERE id='$idget'");
...
if($msg != '' && $msg != ' ' && $msg != "\n\r" && $disabled == '' && $sesschecktwo != '0' && $msgc < 1001) {
$textchk = strlen($msg);
$insert = mysqli_query($conn, "INSERT INTO status_comments (sid, user, ip, colour, message, date, time) VALUES ('".$idget."','".$logged."','".$ip."','".$colour."','".$msg."','".date('d-m-y H:i:s')."','".time()."')");
...
$update = mysqli_query($conn, "UPDATE members SET points='$points'+1 WHERE username='$logged'");
	...
	header('Location: /status/status.php?id='.$idget);
	exit;
}
else {
